...
-- not supported since events_subquery_2 has another subquery inside it
SELECT ("final_query"."event_types") as types, count(*) AS sumOfEventType
FROM
  ( SELECT *, random()
   FROM
               ...
               UNION 
                 (SELECT *
                  FROM
                    (SELECT "events"."user_id", "events"."time", 3 AS event
                     FROM events_table as "events"
                     WHERE event_type IN (26, 27, 28, 29, 30, 13) OFFSET 3) events_subquery_4)) t1
         GROUP BY "t1"."user_id") AS t) "q" 
INNER JOIN
     (SELECT "users"."user_id"
      FROM users_table as "users"
...
FROM     ( 
                  ...
                  SELECT   * , 
                           Row_number() over (PARTITION BY "user_id" ORDER BY "user_id") AS "some_vals",
                           Random() 
                  FROM     events_table offset 3 ) user_id 
ORDER BY 1, 
         2 limit 10;
